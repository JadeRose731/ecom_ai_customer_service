"""售后 MCP Server(ch08 自建,mock 数据:保修期 / 退货进度)。独立进程:
uv run python mcp_servers/aftersales_server.py(:8102)
MOCK_DELAY_SECONDS / PORT 环境变量与物流 Server 同约定。"""
import asyncio
import os
import random
...
from mcp.server.fastmcp import FastMCP   # 实装 mcp 1.30.0 无 MCPServer 高层类,走 FastMCP(dev-notes 裁定)
