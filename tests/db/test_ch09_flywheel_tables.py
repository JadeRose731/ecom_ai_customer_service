...
async def test_insert_low_confidence_with_snapshot(db_session_factory, db_clean):
...
async def test_insert_low_confidence_without_snapshot(db_session_factory, db_clean):
    await repository.insert_low_confidence(None, "没走检索的问题", "user_feedback", None)
    rows = await repository.fetch_unmatched_low_conf(10)
    assert rows[0].retrieved_chunks is None


async def test_review_item_create_and_merge(db_session_factory, db_clean):
...
async def test_set_matched_review_links_raws(db_session_factory, db_clean):
...
async def test_update_review_status_only_from_pending(db_session_factory, db_clean):
    rid = await repository.insert_review_item("猫窝是否支持水洗", None)
...
async def test_list_review_queue_orders_by_occurrence(db_session_factory, db_clean):
...
async def test_eval_runs_roundtrip(db_session_factory, db_clean):
