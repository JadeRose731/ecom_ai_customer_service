# 计划片段没带 db_clean,跨文件跑会互相污染计数断言;统一补上(见 dev-notes/ch03)
...
async def test_mark_vectorized_removes_from_pending(db_session_factory, db_clean):
...
async def test_set_neighbors(db_session_factory, db_clean):
