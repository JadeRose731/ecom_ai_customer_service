# ch04:vectorize_pending 写 BM25 text + 元数据字段,支持 collection 隔离(测试临时集合)。
import time
import uuid
...
@pytest.mark.asyncio
async def test_vectorize_writes_bm25_and_metadata(db_session_factory, db_clean, monkeypatch):
    # 嵌入 mock 成定长向量
